...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        help="comma-separated module names to LoRA-wrap. "
             "Auto-detected if omitted (Qwen-style q_proj/v_proj/k_proj/o_proj "
             "vs BERT-style query/key/value).",
    )
    parser.add_argument(
    ...
    if not args.full_finetune:
        try:
            ...
            target_modules = _resolve_target_modules(
                args.lora_target_modules, model[0].auto_model
            )
            lora = LoraConfig(
                r=args.lora_rank,
...
def _resolve_target_modules(override: str | None, auto_model) -> list[str]:
    """Pick LoRA target modules. Honors --lora-target-modules; otherwise probes
    the loaded model for a known attention-projection naming convention.
    """
    if override:
    ...
    if any(n.endswith("attention.self.query") or n.endswith(".query") for n in names):
